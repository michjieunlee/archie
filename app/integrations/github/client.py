...
class GitHubClient:
    ...
    def __init__(self):
        ...
        self._cached_categories: Optional[List[str]] = None  # Cache for discovered categories
    ...
    async def _scan_directory_for_kb(self, contents) -> List[Dict[str, Any]]:
        ...
        for content in contents:
            if content.type == "dir":
                ...
                subcontents = self.repo.get_contents(content.path, ref=self.default_branch)
    ...
    async def _parse_kb_document(self, file_content) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                'title': frontmatter.get('title', file_content.name.replace('.md', '')) if frontmatter else file_content.name.replace('.md', ''),
                ...
                'ai_confidence': frontmatter.get('ai_confidence') if frontmatter else None,
                'created_date': frontmatter.get('created_date') if frontmatter else None,
                'last_updated': frontmatter.get('last_updated') if frontmatter else None,
        ...
        try:
            ...
            for content in contents:
                if content.type == "dir":
                    ...
                    try:
                        dir_contents = self.repo.get_contents(content.path, ref=self.default_branch)
                        has_markdown = any(item.name.endswith('.md') for item in dir_contents if item.type == "file")
                        if has_markdown:
            ...
            if not self._cached_categories:
                ...
                logger.info("No categories found in repository, using default categories")
    ...
    async def delete_file(self, branch_name: str, file_path: str, commit_message: str) -> str:
        """
