    ...
    try:
        ...
        for i in range(num_documents):
            mock_documents.append(
                {
                    "title": f"Knowledge Document {i+1}: {random.choice(['API Integration', 'Database Schema', 'UI Component', 'Authentication Flow', 'Data Processing'])}",
                    "category": random.choice(categories),
                    "content": f"This is a mock knowledge base document extracted from pull request analysis.\n\n**Context:** Analyzing {owner}/{repo}\n\n**Key Points:**\n- Implementation details\n- Best practices\n- Common issues and solutions",
                    "source_prs": [
                        f"PR#{random.randint(1, 100)}"
